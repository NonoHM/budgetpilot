#!/bin/sh
# Proves the three generated Prisma clients coexist, inside the image that ships them.
#
# Runs *in* a container rather than on the checkout, because the claim being tested is about the
# artifact, not about the source tree. The failure it exists to catch is the one this repo already
# shipped once: all three schemas generated to the same default path and overwrote each other, so
# exactly one client existed at a time. Every job in CI still passed, because each regenerates its
# own client and proves that one works alone. Nothing ever looked at all three together.
#
...
# a check that has never been seen to fail is not yet a check. In the image both defaults hold.
GENERATED_DIR=${GENERATED_DIR:-/app/src/lib/server/database/generated}
BUNDLE_DIR=${BUNDLE_DIR:-/app/build}
