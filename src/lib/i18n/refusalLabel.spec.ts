...
describe('refusalLabel', () => {
	...
	it('renders every code in the union, and there are 41 of them', () => {
		...
		// would satisfy "none is empty" perfectly. 41 since #652 added 'control-character'.
		expect(rendered).toHaveLength(41);
		expect(CSV_REFUSAL_CODES).toHaveLength(41);
		expect(rendered.filter((label) => label.trim().length > 0)).toHaveLength(41);
	...
	it('gives every code its own sentence, so two refusals never read alike', () => {
		...
		expect(new Set(rendered).size).toBe(41);
	});
