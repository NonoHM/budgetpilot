	...
	<section class="mx-auto max-w-5xl space-y-6">
		...
		<div id="bills-list" tabindex="-1" class="space-y-6 outline-none">
			...
			{:else if noStreamsAtAll}
				...
				/>
			{:else if nothingDueThisPeriod && bills.emptyState === 'all-stale'}
				<!-- Every stream that survives exclusions has gone quiet longer than one tolerated
				     ...
				     ANY month — see `isStreamStale`/`buildBillOccurrences`. `emptyState` is computed
				     over the whole 12-month lookback, not this period alone, so a month that DOES hold
				     one of these streams' already-realized rows never reaches this branch (`rows` is
				     non-empty there, `nothingDueThisPeriod` is false). -->
				<EmptyState
					icon={emptyIcon}
