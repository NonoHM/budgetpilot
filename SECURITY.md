...
| Availability                              | 3          | The backup export and `/reports` still have no rate limit, and nothing bounds concurrent imports ([#283](https://github.com/NonoHM/budgetpilot/issues/283)); the three import doors are now rate limited. **Three** measured denial-of-service paths are closed, not two: the spreadsheet import bounds what a file expands to ([#254](https://github.com/NonoHM/budgetpilot/issues/254)), the backup restore bounds a payload's structure before parsing it ([#276](https://github.com/NonoHM/budgetpilot/issues/276)), and the CSV import now bounds the dimensions a parsed file may reach before any consumer reads it. The third was measured after this row first claimed two, and the row said two for as long as it was wrong |
